...
* UT - Identificador de artigo exclusivo
...
net=networkPlot(NetMatrix,  normalize = "salton", weighted=NULL, n = 100, Title = "Acoplamento de Autores", type = "fruchterman", size=5,size.cex=T,remove.multiple=TRUE,labelsize=0.8,label.n=10,label.cex=F)
```

### Análise de acoplamento de autores

Realiza uma análise de rede de acoplamento e plota os resultados da detecção da comunidade em um mapa bidimensional (Mapa de Acoplamento). A análise pode ser realizada em três unidades diferentes: documentos, autores ou fontes e a força de acoplamento pode ser medida usando a abordagem clássica (acoplada por referências) ou uma nova abordagem baseada no conteúdo da unidade (palavras-chave ou termos de títulos e resumos)
...
### Mapa Temático por co-palavras

O mapa temático é baseado na análise e agrupamento de redes de co-palavras. A metodologia é inspirada na proposta de Cobo et al. (2011).
...
Nos ajuda a visualizar os principais itens (10 por exemplo) de três campos (autores, palavras-chave, E afiliação) e como eles estão relacionados por meio de um diagrama de Sankey.

```{r}
threeFieldsPlot(M, fields = c("AU", "DE", "C1"), n = c(10, 10, 10))
```
